...
public class UserServiceImpl implements UserService {
    ...
    @Override
    public List<StaffResponseDTO> getStaffByBuilding(Long buildingId){
        List<UserEntity> staffs = userRepository.findByStatusAndRoles_Code(1,"STAFF");
        List<UserEntity> staffsByBuilding = userRepository.findStaffByBuilding(buildingId);
        List<StaffResponseDTO> results = checkStaffAssignment(staffs,staffsByBuilding);
        return results;
    }

    @Override
    public List<StaffResponseDTO> getStaffByCustomer(Long customerId){
        List<UserEntity> staffs = userRepository.findByStatusAndRoles_Code(1,"STAFF");
        List<UserEntity> staffsByCustomer = userRepository.findStaffByCustomer(customerId);
        List<StaffResponseDTO> results = checkStaffAssignment(staffs,staffsByCustomer);
        return results;
    }

    private List<StaffResponseDTO> checkStaffAssignment(List<UserEntity> staffs, List<UserEntity> staffsByBuilding) {
        ...
        for(UserEntity item : staffs){
            ...
            if(staffsByBuilding.contains(item)){
